...
/**
 * One-time migration: copy/move subdirectories from the old extension's globalStorage to the new one.
 *
 * When the extension name changes (e.g. cline-cn → cline-cn-ai), VS Code creates a
 * fresh globalStorageUri for the new extension ID. This function migrates data
 * per-directory so that task history, checkpoints, and other large data are preserved:
 *
 * - `settings`, `state`, `tasks` — **copied** (old data kept for safety)
 * - `checkpoints` — **moved** (cut to save disk space, as git repos can be large)
 *
 * The sentinel is only set when all operations succeed; on errors the migration
 * will retry on the next startup.
 */
async function migrateGlobalStorageIfNeeded(context: vscode.ExtensionContext): Promise<void> {
